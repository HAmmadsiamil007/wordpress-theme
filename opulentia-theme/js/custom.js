/**
 * Custom JavaScript for opulentia theme
 *
...
(function () {
    ...
    function initReadingProgress() {
        ...
        function updateProgress() {
            ...
            // Update aria-valuenow on the container
            var container = bar.closest('.reading-progress');
            if (container) {
    ...
    function initVideoPopups() {
        document.querySelectorAll('.opulentia-video-popup').forEach(function (container) {
            ...
            var embedUrl = playBtn.dataset.embed || '';
    ...
    /* ------------------------------------------------
     *  Blog Animations (GSAP)
     * ------------------------------------------------ */
    function initBlogAnimations() {
        if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;

        // Animate post cards as they scroll into view
        var postGrid = document.querySelector('.posts-grid');
        if (postGrid) {
            var posts = postGrid.querySelectorAll('.post-card, .post-list, .post-classic');
            if (posts.length > 0) {
                gsap.from(posts, {
                    y: 50,
                    opacity: 0,
        ...
        // Animate related post cards
        var relatedGrid = document.querySelector('.related-posts__grid');
        if (relatedGrid) {
            gsap.from(relatedGrid.querySelectorAll('.related-post-card'), {
                y: 40,
                opacity: 0,
                stagger: 0.12,
                duration: 0.7,
        ...
        // Animate reading progress bar (initial reveal)

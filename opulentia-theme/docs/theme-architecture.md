...
opulentia is a luxury dark-themed WooCommerce WordPress theme for an Italian footwear brand. Premium aesthetic with gold accents, serif headings, and GSAP-powered scroll animations.
...
│
├── inc/                          # Core includes
│   ├── class-opulentia-after-setup.php    # Singleton: theme setup, CPTs, taxonomies
│   ├── class-opulentia-enqueue.php        # Singleton: scripts/styles/Vite/GSAP
...
│
├── languages/                    # i18n
│   └── opulentia.pot           # Translation template
│
├── docs/                         # Documentation
...
All enqueued via `Opulentia_Enqueue` singleton with `defer` attribute. Custom animations in `js/custom.js`.
...
| Class | File | Status |
|---|---|---|
| `Opulentia_After_Setup` | `inc/class-opulentia-after-setup.php` | ✅ Complete |
| `Opulentia_Enqueue` | `inc/class-opulentia-enqueue.php` | ✅ Complete |
| `Opulentia_Icons` | — | ⬜ Planned |
| `Opulentia_Customizer_Config` | — | ⬜ Planned |
| `Opulentia_Dynamic_CSS` | — | ⬜ Planned |
...
- Vite HMR helpers are private methods on `Opulentia_Enqueue`
...
Planned: Refactor into `inc/class-opulentia-customizer-config.php` with per-section config files in `inc/customizer/configurations/`.
...
1. **Duplicate `Opulentia_pingback_header()`** — defined in both `inc/theme-setup.php` AND `inc/template-functions.php`. Will cause fatal error if both load without guards. Currently mitigated by early-return in `theme-setup.php` when class is active. Fix: remove from `template-functions.php` (Step 11 in migration plan).
...
| Step | Task | Status |
|------|------|--------|
| 1 | `inc/class-opulentia-after-setup.php` | ✅ |
| 2 | `inc/class-opulentia-enqueue.php` | ✅ |
| 3 | `functions.php` — class requires + init, flat file guards | ✅ |
| 4 | `inc/class-opulentia-icons.php` — SVG icon utility | ⬜ |
| 5 | `inc/class-opulentia-customizer-config.php` | ⬜ |

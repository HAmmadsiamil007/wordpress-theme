...
class Opulentia_Custom_Fonts_Uploader {
    ...
    public function register_font_cpt() {
        register_post_type( 'op_custom_font', array(
            'labels' => array(
    ...
    public function render_meta_box( $post ) {
        ...
        (function($){
            ...
            $('#op-font-add-row').on('click', function(){
                var html = '<tr>' +
                    '<td><select name="_op_font_variants[' + idx + '][weight]" style="width:100%"><?php foreach ( array( '100', '200', '300', '400', '500', '600', '700', '800', '900' ) as $w ) : ?><option value="<?php echo $w; ?>"><?php echo $w; ?></option><?php endforeach; ?></select></td>' +
    ...
    public function save_meta_box( $post_id ) {
        ...
        if ( ! isset( $_POST['op_custom_font_files_nonce'] ) ) { return; }
        if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['op_custom_font_files_nonce'] ) ), 'op_custom_font_files' ) ) { return; }
    ...
    public function admin_enqueue( $hook ) {
        ...
        if ( 'op_custom_font' !== get_post_type() ) { return; }
    }
    ...
    private function output_preload_hints( $fonts ) {
        foreach ( $fonts as $font ) {
            if ( ! get_post_meta( $font->ID, '_op_font_preload', true ) ) { continue; }
            $variants = (array) get_post_meta( $font->ID, '_op_font_variants', true );
            foreach ( $variants as $v ) {

...
class Opulentia_GDPR_Cookie_Consent {
    ...
    public function customize_register( $wp_customize ) {
        $wp_customize->add_panel( 'opulentia_gdpr', array(
        ...
        // ── Bar Appearance ──
        $wp_customize->add_section( 'op_gdpr_bar', array(
            'title' => __( 'Bar', 'opulentia' ),
            'panel' => 'opulentia_gdpr',
        ) );

        $wp_customize->add_setting( 'op_gdpr_enable', array(
            'default'           => false,
            'sanitize_callback' => 'wp_validate_boolean',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_enable', array(
            'label'   => __( 'Enable Cookie Consent Bar', 'opulentia' ),
            'section' => 'op_gdpr_bar',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'op_gdpr_position', array(
            'default'           => 'bottom',
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_position', array(
        ...
        $wp_customize->add_setting( 'op_gdpr_message', array(
            'default'           => __( 'We use cookies to improve your experience. By continuing, you agree to our use of cookies.', 'opulentia' ),
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_message', array(
            'label'   => __( 'Message', 'opulentia' ),
            'section' => 'op_gdpr_bar',
            'type'    => 'text',
        ) );

        $wp_customize->add_setting( 'op_gdpr_accept_text', array(
            'default'           => __( 'Accept All', 'opulentia' ),
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_accept_text', array(
            'label'   => __( 'Accept All Button', 'opulentia' ),
            'section' => 'op_gdpr_bar',
            'type'    => 'text',
        ) );

        $wp_customize->add_setting( 'op_gdpr_customize_text', array(
            'default'           => __( 'Customize', 'opulentia' ),
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_customize_text', array(
            'label'   => __( 'Customize Button', 'opulentia' ),
            'section' => 'op_gdpr_bar',
            'type'    => 'text',
        ) );

        $wp_customize->add_setting( 'op_gdpr_save_text', array(
            'default'           => __( 'Save Preferences', 'opulentia' ),
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_save_text', array(
        ...
        // ── Cookie Policy Link ──
        $wp_customize->add_setting( 'op_gdpr_policy_page', array(
            'default'           => 0,
            'sanitize_callback' => 'absint',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_policy_page', array(
        ...
        // ── Categories ──
        $wp_customize->add_section( 'op_gdpr_categories', array(
        ...
        // ── Colors ──
        $wp_customize->add_section( 'op_gdpr_colors', array(
            'title' => __( 'Colors', 'opulentia' ),
            'panel' => 'opulentia_gdpr',
        ) );

        $wp_customize->add_setting( 'op_gdpr_bg_color', array(
            'default'           => '#1a1a1a',
            'sanitize_callback' => 'sanitize_hex_color',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'op_gdpr_bg_color', array(
            'label'   => __( 'Background Color', 'opulentia' ),
            'section' => 'op_gdpr_colors',
        ) ) );

        $wp_customize->add_setting( 'op_gdpr_text_color', array(
            'default'           => '#f5f5f5',
            'sanitize_callback' => 'sanitize_hex_color',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'op_gdpr_text_color', array(
            'label'   => __( 'Text Color', 'opulentia' ),
            'section' => 'op_gdpr_colors',
        ) ) );

        $wp_customize->add_setting( 'op_gdpr_accent_color', array(
            'default'           => '#c9a96e',
            'sanitize_callback' => 'sanitize_hex_color',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'op_gdpr_accent_color', array(
            'label'   => __( 'Accent / Button Color', 'opulentia' ),
            'section' => 'op_gdpr_colors',
        ) ) );

        // ── Cookie Lifespan ──
        $wp_customize->add_section( 'op_gdpr_advanced', array(
            'title' => __( 'Advanced', 'opulentia' ),
            'panel' => 'opulentia_gdpr',
        ) );

        $wp_customize->add_setting( 'op_gdpr_cookie_lifespan', array(
            'default'           => 365,
            'sanitize_callback' => 'absint',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_cookie_lifespan', array(
            'label'   => __( 'Cookie Lifetime (days)', 'opulentia' ),
            'section' => 'op_gdpr_advanced',
            'type'    => 'number',
            'input_attrs' => array( 'min' => 1, 'max' => 3650 ),
        ) );
    }

    private function register_category( $wp_customize, $id, $label, $default_checked, $required ) {
        $wp_customize->add_setting( 'op_gdpr_cat_' . $id . '_title', array(
            'default'           => $label,
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_cat_' . $id . '_title', array(
            'label'   => sprintf( __( 'Category: %s Title', 'opulentia' ), $id ),
            'section' => 'op_gdpr_categories',
            'type'    => 'text',
        ) );

        $wp_customize->add_setting( 'op_gdpr_cat_' . $id . '_desc', array(
            'default'           => $this->default_category_desc( $id ),
            'sanitize_callback' => 'sanitize_text_field',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_gdpr_cat_' . $id . '_desc', array(
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script( 'opulentia-gdpr', 'OpulentiaGDPR', array(
            ...
            <div class="op-gdpr-bar__preferences" id="op-gdpr-preferences" style="display:none;">
                <?php foreach ( $cats as $cat ) : ?>
                    ...
                    <label class="op-gdpr-bar__cat">
                        <input type="checkbox" name="gdpr_<?php echo esc_attr( $cat ); ?>" value="1" <?php checked( $required ); disabled( $required ); ?>>

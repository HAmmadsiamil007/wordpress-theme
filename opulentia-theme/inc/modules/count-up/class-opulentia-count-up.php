...
';
    ...
    public function customize_register( $wp_customize ) {
        $wp_customize->add_section( 'opulentia_count_up', array(
            'title'    => __( 'Count-Up Numbers', 'opulentia' ),
            'panel'    => 'Opulentia_global_settings',
            'priority' => 215,
        ) );

        $wp_customize->add_setting( 'op_countup_enable', array(
            'default'           => true,
            'sanitize_callback' => 'wp_validate_boolean',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_countup_enable', array(
            'label'   => __( 'Enable Count-Up Numbers', 'opulentia' ),
            'section' => 'opulentia_count_up',
            'type'    => 'checkbox',
        ) );

        $wp_customize->add_setting( 'op_countup_duration', array(
            'default'           => 2000,
            'sanitize_callback' => 'absint',
            'transport'         => 'refresh',
        ) );

        $wp_customize->add_control( 'op_countup_duration', array(

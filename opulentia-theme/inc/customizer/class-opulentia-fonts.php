 ...
 *
 * Central class for Google Fonts integration:
 * - Provides a curated list of 300+ Google Fonts for customizer selectors
 * - Builds optimized Google Fonts CSS URL with display=swap
 * - Manages font subset selection
 * - Handles font weight arrays for each family
 * - Provides preconnect/preload hints
 *
 * @package Opulentia
 * @since   2.0.0
 */
...
/**
 * Opulentia_Fonts class.
 */
class Opulentia_Fonts {
    ...
    /**
     * Output preconnect links for Google Fonts CDN.
     */
    ...
    /**
     * Build the full Google Fonts CSS URL from theme settings.
     *
     * Reads typography settings via Opulentia_get_option() and compiles
     * all required families with their weights into one Google Fonts URL.
     *
     * @return string Google Fonts CSS URL or empty string.
     */
    public function get_google_fonts_url() {
        $families = $this->get_active_font_families();
        ...
        // Add subset if set.
        $subset = Opulentia_get_option( 'typo-subset', '' );
        if ( ! empty( $subset ) ) {
    ...
    /**
     * Get all active font families with their weights from current settings.
     *
     * Reads all typography font family settings and collects unique
     * font families with their required weight variants.
     *
     * @return array Array of family:ital,wght@... strings.
     */
    public function get_active_font_families() {
        $families = array();

        // Font family settings to scan.
        $family_settings = array(
            'typo-headings-family' => array( 300, 400, 500, 600, 700 ),
            'typo-body-family'     => array( 300, 400, 500, 600, 700 ),
        );

        // Add per-heading font families.
        foreach ( array( 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' ) as $tag ) {
            $family_settings[ 'typo-' . $tag . '-family' ] = array( 300, 400, 500, 600, 700 );
        }

        // Add secondary families.
        foreach ( array( 'site-title', 'tagline', 'nav', 'btn', 'widget' ) as $prefix ) {
            $family_settings[ 'typo-' . $prefix . '-family' ] = array( 300, 400, 500, 600, 700 );
        }

        $collected = array();

        foreach ( $family_settings as $setting => $weights ) {
            $font_family = Opulentia_get_option( $setting, '' );

            if ( empty( $font_family ) || $this->is_system_font( $font_family ) ) {
                continue;
            }

            $slug = str_replace( ' ', '+', trim( $font_family ) );

            if ( ! isset( $collected[ $slug ] ) ) {
                $collected[ $slug ] = array();
            }

            // Merge weights from the global font setting
            // and control-specific settings.
            $weight_setting = str_replace( '-family', '-weight', $setting );
            $weight = Opulentia_get_option( $weight_setting, '' );

            if ( ! empty( $weight ) && is_numeric( $weight ) ) {
                $collected[ $slug ][] = (int) $weight;
            }

            // Also add standard weights.
            foreach ( $weights as $w ) {
                $collected[ $slug ][] = $w;
            }
        }

        // Build the <family>:wght@... strings.
        $families = array();
        foreach ( $collected as $slug => $weights ) {
            $weights = array_unique( array_map( 'intval', $weights ) );
            sort( $weights );

            if ( in_array( 400, $weights, true ) && count( $weights ) === 1 ) {
                // Single weight — can omit the wght param
                $families[] = $slug;
            } else {
                $families[] = $slug . ':wght@' . implode( ';', $weights );
            }
        }
    ...
    /**
     * Check if a font name is a known system font stack (not a Google Font).
     *
    ...
    private function is_system_font( $font ) {
        ...
        return in_array( strtolower( trim( $font ) ), array_map( 'strtolower', $system_fonts ), true );
    }

    /**
     * Get a list of popular Google Fonts for use in customizer selectors.
     *
     * @return array Font name => Font name.
     */
    public static function get_google_fonts_list() {
        return apply_filters( 'Opulentia_google_fonts_list', array(
            ''                    => __( '— Select Font —', 'opulentia' ),
            'ABeeZee'             => 'ABeeZee',
            'Abel'                => 'Abel',
